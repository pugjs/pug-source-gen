...
CodeGenerator.prototype = {
  ...
  attrs: function(attrs) {
    ...
    attrs.forEach(function(attr) {
      ...
      if (attr.name === 'class' && !attr.mustEscape && constVal &&
                 /^\-?[_a-z][_a-z0-9\-]*$/i.test(constVal)) {
        classes += '.' + constVal;
      } else if (attr.name === 'id' && !id && !attr.mustEscape && constVal &&
                 /^[\w-]+$/.test(constVal)) {
        id = constVal;
      } else {
        var attrOut = '';
  ...
  visitTag: function(tag, inline, parent) {
    ...
    // tag name
    if (tag.type === 'InterpolatedTag') {
      buf += '#{' + tag.expr + '}';
    } else if (tag.selfClosing || tag.name !== 'div' || attrs[0] !== '.' && attrs[0] !== '#') {
      buf += tag.name;
    }
  ...
  visitInterpolatedTag: function(tag, inline, parent) {
    this.visitTag(tag, inline, parent);
  },
  ...
  visitCode: function(code, inline, parent) {
    ...
    if (inline && parentBlock && parentBlock.nodes.length !== 1) {
      if (code.buffer) this.buffer((code.mustEscape ? '#' : '!' ) + '{' + code.val + '}');
  ...
  visitRawInclude: function(rawInclude) {
    this.bufLine('include');
    rawInclude.filters.forEach(function(includeFilter) {
      this.visit(includeFilter);
    }.bind(this));
    this.buffer(' ');
    this.visit(rawInclude.file);
  },

  visitIncludeFilter: function(includeFilter){
    this.buffer(':' + includeFilter.name + this.attrs(includeFilter.attrs));
  },
